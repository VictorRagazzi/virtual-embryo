...
from .data import (load_dictionary, read_gene_map, read_stage,
                   select_future_neighbors, split_cells)
...
def build_pairs(stages, splits, components, seed):
    ...
    for start, end in itertools.combinations(sorted(stages), 2):
        for split, label in enumerate(["train", "validation"]):
            ...
            neighbors, distances = select_future_neighbors(coordinates[start][source_indices], coordinates[end][target_indices])
...
def train(args):
    ...
    for specification in args.stage:
        ...
        stages[day] = read_stage(path, args.expression_scale, tokens, medians,
                                 args.max_length, gene_map, args.layer, args.max_cells, args.seed)
    if len(stages) < 2:
    ...
    for epoch in range(args.epochs):
        ...
        if metrics["validation_mse"] < best:
            ...
            torch.save({"state_dict": model.state_dict(), "encoder_config": encoder.config.to_dict(),
                        ...
                        "trainable_layers": args.trainable_layers, "stages": sorted(stages)}, args.output / "best.pt")
...
def main():
    ...
    training.add_argument("--stage", action="append", required=True, help="DIA=arquivo.h5ad; repetir para cada estágio")
    ...
    training.add_argument("--max-cells", type=int)
