...
def select_future_neighbors(source_expression, future_expression):
...
def read_stage(path, scale, token_dictionary, gene_medians, max_length, gene_map=None,
               layer=None, max_cells=None, seed=42):
    ...
    linear, expression = prepare_expression(subset.X if layer is None else subset.layers[layer], scale)
